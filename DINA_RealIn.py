...
def em(X, Q, maxIter=10000, tol=1e-3, prior=None):
    n_stu = X.shape[0]
    n_qus = X.shape[1]
    n_kno = Q.shape[0]
...
def input_A(n_stu,A_all,filepath):
    A=np.zeros((n_stu,A_all.shape[1]))
    A= pd.read_csv(filepath).values
...
def evaluate(X, Q, priors):
    ...
    for i in range(n):
        pi, g, s, gamma = em(X, Q, maxIter=1000, tol=1e-6, prior=priors[i])
...
def test_real(n_stu, n_qus, n_kno, priors, filepath,randomState=1):
    # 设置随机数种子，使得实验结果可重现
    np.random.seed(randomState)


    #输入Q矩阵
    Q = np.mat(pd.read_csv(filepath+'/q.csv').head(15).values)
    Q = np.array(Q)
    ...
    A_all = np.array(list(product([0, 1], repeat=n_kno)))
    #输入真实数据
    A, A_idx =input_A(n_stu,A_all,(filepath+'/alpha.csv'))
   ...
   #results.append((pi, g, s, gamma, A, A_idx))   results构成
    results = evaluate(X, Q, priors)
    # print(len(results[1][4]))
...
def test1(n_stu, n_qus, n_kno, distribution=1, times=100):
    ...
    return df_result
def test2(n_stu, n_qus, n_kno, filepath, times=100):
    pmrs = np.zeros(shape=(4, times))
    mmrs = np.zeros(shape=(4, times))
    sr2s = np.zeros(shape=(4, times))
    gr2s = np.zeros(shape=(4, times))
    ...
    # 获取所有可能的认知状态
    A_all = np.array(list(product([0, 1], repeat=n_kno)))
    priors = get_priors(A_all, p_know=0.7, p_know_list=[0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.7])

    for i in tqdm(range(times)):
        df_single = test_real(n_stu, n_qus, n_kno, priors, filepath)
        pmrs[:, i] = df_single.pmr
        mmrs[:, i] = df_single.mmr
        sr2s[:, i] = df_single.s_r2
        gr2s[:, i] = df_single.g_r2
...
print("data scale: 10")
print("")
print(test2(n_stu=10 ,n_qus=15, n_kno=11,filepath='Math1_1'))
print("data scale: 100")
print("")
print(test2(n_stu=100 ,n_qus=15, n_kno=11,filepath='Math1_2'))
print("data scale: 1000")
print("")
print(test2(n_stu=1000 ,n_qus=15, n_kno=11,filepath='Math1_3'))


################要记得再p_know_list里面改，知识点数量变了就要改
