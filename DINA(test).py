...
#四种先验
for i in range(4):
    pi_t, g_t, s_t, gamma = em(X0,Q, maxIter=10000, tol=1e-6, prior=priors[i])
    ss.append(s_t)
    gs.append(g_t)
    A, A_idx = solve(gamma, Q.shape[0])
    ...
    print("先验模式")
    print(i)
    results = evaluate(X1, Q, priors)
    df_result = score(results, A, A_idx, s_t, g_t)
    print("this :",i)
    print(df_result)

    # 联合概率对数似然
    # 联合对数似然表达了观察到的答题数据 X 在给定潜在技能状态（和参数）的情况下的“合理性”，值越大，表示模型假设下观察数据出现的可能性越大。
    LL = joint_loglike(X0,Q,s_t,g_t,pi_t)
#     # 边缘概率对数似然
...
# 负对数似然（NLL）被添加到 NLL 列表中，用于衡量模型对数据的拟合优度，数值越低越好.;,  , ,
# AIC 通过公式 AIC = -2 * LL + d[i] 计算，用于综合考虑模型的复杂度和拟合优度，AIC 越小越好
